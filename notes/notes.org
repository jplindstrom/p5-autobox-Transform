...
sort {
    ...
    ||
    int( $b->{age} / 10 ) <=> int( $a->{age} / 10 ) # reverse
} @users
**** With order
***** provide an order spec of how one value should be compared with another value
****** the value can be transformed using an optional subref
******* e.g. by looking up a hashref key
****** how to compare (cmp or <=>)
****** which direction to sort (ascending or descending)
***** in case of a tie, provide another order spec
***** example with order
order(
    sub { $_->{name} },
    [ "num", "desc", sub { int( $_->{age} / 10 ) } ],
)
***** example with order_by
order_by(
    name => "str",
    age  => [ "num", "desc", sub { int( $_ / 10 ) } ],
)
*** order options
**** one or many options?
***** if there's only one option, provide a scalar or subref
***** if there are many options, provide them in an arrayref in any order
